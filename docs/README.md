...
| | |
|---|---|
| [`reports/`](reports/) | **Delivery reports — the best place to start.** One per milestone, each stating what was asked for, what was delivered, what was *not* delivered, and the measurements behind every claim. They are written to be read by someone who wasn't there. [`reports/m19-review.md`](reports/m19-review.md) is the most recent; [`reports/m14.md`](reports/m14.md) is the most representative. |
| [`decisions/`](decisions/) | **Architecture decision records.** Fourteen of them, each recording a choice, the alternatives measured, and the conditions under which the choice should be revisited. [ADR 0001](decisions/0001-kicad-io.md) (why we write KiCad files ourselves) and [ADR 0009](decisions/0009-pours.md) (what `pcbnew` can and cannot do headlessly) are the two most load-bearing. |
| [`notes/`](notes/) | **Study notes.** Research written up outside the milestone cycle — reading something the project did not build, to decide what to take from it. [`notes/toporouter-postmortem.md`](notes/toporouter-postmortem.md) reads gEDA PCB's rubber-band router, the only complete open-source predecessor aipcb's stretcher has, for both its techniques and its failure. [`notes/routing-literature.md`](notes/routing-literature.md) reads the tightening, congestion and machine-learning literatures against the router's own profile, and finds that the funnel algorithm three milestones called the hot spot costs 0.13% of a route. |
