        ...
        "pcie-sata": frozenset(
            {
                ...
                # `route-doubles-back` was here from M16b until M17b, and it is
                # gone rather than silenced: GND's `U1.17>U1.49` no longer hops to
                # B.Cu for half a millimetre and retraces its own path home,
                # because M17's via pass asked whether that span fitted on F.Cu in
                # one leg and it did -- two vias and 17.116 mm of copper back.
                # `TestSelfCrossingInvariant` still routes this board and still
                # asserts the count, at zero now, so the guard that found it is
